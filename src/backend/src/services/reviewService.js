...
exports.fetchAggregatedReviews = async (isbn) => {
  ...
  if (!isbn || String(isbn).length < 10) return aggregated;

  // 1. Primary: Try Google Books API for Official Editorial Synopsis
  try {
    const googleRes = await axios.get(`https://www.googleapis.com/books/v1/volumes?q=isbn:${isbn}`, { timeout: 6000 });
    ...
    console.warn(`[ReviewService] Google Books synopsis fetch failed for ${isbn}`, error.message);
  }
  ...
  if (aggregated.length === 0) {
    try {
      ...
      console.warn(`[ReviewService] Failed to scrape Goodreads fallback for ${isbn}`, error.message);
    }
  }
